...
const variantClasses: Record<string, string> = {
  ...
  muted: 'bg-muted text-muted-foreground border border-muted/30',
  secondary: 'bg-secondary/10 text-secondary border border-secondary/30',
  info: 'bg-info/10 text-info border border-info/30',
};

/**
 * Bloom Elements badge component (pill style)
 * Padrão: text-[9px] font-display font-bold rounded-lg uppercase tracking-widest
 */
export function BloomBadge({
  ...
  return (
    <span
      className={`text-[9px] font-display font-bold rounded-lg uppercase tracking-widest px-2.5 py-1 ${variantClasses[variant]} ${className}`}
    >
      {children}
    </span>
  );
