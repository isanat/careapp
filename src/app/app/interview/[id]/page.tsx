...
export default function InterviewPage({
        ...
        if (response.ok) {
          ...
          if (data.interview.questionnaire) {
            setCommunicationRating(
              data.interview.questionnaire.communicationRating || 3,
            );
            setExperienceRating(
              data.interview.questionnaire.experienceRating || 3,
            );
            setPunctualityRating(
              data.interview.questionnaire.punctualityRating || 3,
            );
            setWouldRecommend(
              data.interview.questionnaire.wouldRecommend ?? true,
            );
            setProceedWithContract(
              data.interview.questionnaire.proceedWithContract ?? false,
            );
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (proceedWithContract && data.contractId) {
          router.push(
            `/app/contracts/new?caregiverId=${interview.caregiverUserId}`,
          );
        } else {
  ...
  const getStatusBadge = () => {
    switch (interview.status) {
      ...
      case "IN_PROGRESS":
        return (
          <span className="text-[9px] font-display font-bold rounded-lg uppercase tracking-widest px-2.5 py-1 bg-success/10 text-success border border-success/30">
            Em Andamento
          </span>
        );
      case "COMPLETED":
        return (
          <span className="text-[9px] font-display font-bold rounded-lg uppercase tracking-widest px-2.5 py-1 bg-primary/10 text-primary border border-primary/30">
            Concluida
          </span>
        );
      case "CANCELLED":
        return (
          <span className="text-[9px] font-display font-bold rounded-lg uppercase tracking-widest px-2.5 py-1 bg-destructive/10 text-destructive border border-destructive/30">
            Cancelada
          </span>
        );
      default:
  ...
  return (
    <AppShell hideBottomNav={interview.status === "IN_PROGRESS"}>
      <div className="space-y-6 max-w-4xl mx-auto pb-6">
            ...
            <div className="bg-card rounded-3xl p-5 sm:p-7 border border-border shadow-card">
              ...
              <div className="space-y-3">
                {[
                  {
                    icon: "\uD83C\uDFA5",
                    text: "Verifique se sua câmera e microfone funcionam",
                  },
                  {
                    icon: "\uD83D\uDD07",
                    text: "Escolha um local silencioso e bem iluminado",
                  },
                  {
                    icon: "\uD83D\uDCDD",
                    text: "Tenha suas perguntas anotadas",
                  },
                  {
                    icon: "\uD83D\uDD12",
                    text: "A entrevista é privada e segura",
                  },
                ].map((tip, i) => (
                  <div
                    key={i}
              ...
              <div className="flex items-start gap-4 p-5 bg-warning/5 border border-warning/20 rounded-2xl">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Para finalizar a entrevista e ir para feedback, clique em
                    "Finalizar" abaixo.
                  </p>
                </div>
              </div>
          ...
          !interview.familyCompletedAt && (
            <div className="space-y-6 max-w-lg mx-auto">
              <div className="space-y-2">
                <h2 className="text-xl sm:text-2xl font-display font-black text-foreground uppercase tracking-tighter mb-6">
                  Feedback da Entrevista
                </h2>
                <p className="text-sm leading-relaxed text-muted-foreground">
                  Avalie a entrevista com {interview.caregiverName}. Este
                  registro protege ambas as partes.
                </p>
              </div>
                ...
                <div className="bg-card rounded-3xl p-5 sm:p-7 border border-border shadow-card space-y-4">
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    <span className="text-sm leading-relaxed">
                      Confirmo que a entrevista em video foi realizada e que
                      ambas as partes participaram de forma voluntaria.
                    </span>
                  </label>
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    />
                    <span className="text-sm leading-relaxed">
                      Discutimos as expectativas de cuidado, horarios e
                      responsabilidades durante a entrevista.
                    </span>
                  </label>
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    <Checkbox
                      ...
                      onCheckedChange={(c) =>
                        setNoMisrepresentation(c === true)
                      }
                      className="mt-0.5"
                    />
                    <span className="text-sm leading-relaxed">
                      Declaro que as informacoes que forneci sao verdadeiras e
                      que nao houve distorcao de informacoes por qualquer parte.
                    </span>
                  </label>
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    <Checkbox
                      ...
                      onCheckedChange={(c) =>
                        setPlatformLiabilityAck(c === true)
                      }
                      className="mt-0.5"
                    />
                    <span className="text-sm leading-relaxed">
                      Entendo que a plataforma atua apenas como intermediaria
                      para conectar familias e cuidadores, nao sendo responsavel
                      pela qualidade ou resultado dos servicos prestados.
                    </span>
                  </label>
                </div>
              </section>
              ...
              <section className="space-y-4">
                ...
                >
                  <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    >
                      <IconContract
                        className={`h-6 w-6 mx-auto mb-1 ${proceedWithContract ? "text-primary" : "text-muted-foreground"}`}
                      />
                      <span
                        className={`text-sm font-display font-bold block ${proceedWithContract ? "text-primary" : "text-foreground"}`}
                      >
                        Criar Contrato
                      </span>
                      <span className="text-[9px] text-muted-foreground">
                        Prosseguir com este cuidador
                      </span>
                    </button>
                    <button
                    ...
                    >
                      <IconClock
                        className={`h-6 w-6 mx-auto mb-1 ${!proceedWithContract ? "text-muted-foreground" : "text-muted-foreground/50"}`}
                      />
                      <span
                        className={`text-sm font-display font-bold block ${!proceedWithContract ? "text-foreground" : "text-muted-foreground"}`}
                      >
                        Ainda nao
                      </span>
                      <span className="text-[9px] text-muted-foreground">
              ...
              <section className="space-y-4">
                <h4 className="text-xs font-display font-bold text-foreground uppercase tracking-widest">
                  Observacoes sobre a entrevista (opcional)
                </h4>
                <div className="bg-card rounded-3xl p-5 sm:p-7 border border-border shadow-card space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Estas notas ficam registradas para sua referencia e
                    seguranca.
                  </p>
                </div>
              </section>
          ...
          <div className="space-y-6 max-w-3xl mx-auto">
            ...
            <div className="bg-success/5 rounded-3xl border border-success/20 p-5 sm:p-7">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h2 className="text-lg sm:text-xl font-display font-black text-success uppercase mb-2">
              ...
              <div className="bg-card rounded-3xl border border-border shadow-card p-5 sm:p-7 space-y-4">
                      ...
                      <div className="flex gap-0.5">
                        {[1, 2, 3, 4, 5].map((i) => (
                          <IconStar
                            ...
                            className={`h-4 w-4 ${
                              i <=
                              (interview.questionnaire?.experienceRating || 0)
                                ? "fill-amber-400 text-amber-400"
                                : "text-muted-foreground/20"
                            }`}
                          />
                ...
                {interview.questionnaire.wouldRecommend !== undefined && (
                  <div className="flex items-center gap-2 pt-2 border-t border-border/50">
                    ...
                    <Badge
                      className={
                        interview.questionnaire.wouldRecommend
                          ? "bg-green-500"
                          : "bg-red-500"
                      }
                    >
              ...
              {interview.questionnaire?.proceedWithContract ? (
                <div className="space-y-2">
                  <p className="text-sm text-muted-foreground">
                    Você decidiu prosseguir com o contrato. O próximo passo é
                    formalizar o acordo.
                  </p>
                  <Button asChild className="w-full h-12 rounded-xl mt-3">
                    <Link
                      href={`/app/contracts/new?caregiverId=${interview.caregiverUserId}`}
                    >
                ...
                <p className="text-sm text-muted-foreground">
                  Você decidiu não prosseguir no momento. Pode entrar em contato
                  com {interview.caregiverName} para discussões futuras.
                </p>
              )}
            </div>
        ...
        {interview.status === "COMPLETED" &&
          !isFamily &&
          !interview.caregiverCompletedAt && (
            <div className="space-y-6 max-w-lg mx-auto">
              <div className="space-y-2">
                <h2 className="text-xl sm:text-2xl font-display font-black text-foreground uppercase tracking-tighter mb-6">
                  Feedback da Entrevista
                ...
                <p className="text-sm leading-relaxed text-muted-foreground">
                  Avalie a entrevista com a familia {interview.familyName}. Este
                  registo protege ambas as partes.
                </p>
              </div>
              ...
              <section className="space-y-4">
                ...
                <div className="bg-card rounded-3xl p-5 sm:p-7 border border-border shadow-card space-y-4">
                  <StarRating
                    label="Respeito e cordialidade"
                    value={cgFamilyRating}
                    onChange={setCgFamilyRating}
                  />
                  <StarRating
                    label="Clareza nas expectativas"
                    value={cgClarityRating}
                    onChange={setCgClarityRating}
                  />
                </div>
              </section>
                ...
                <div className="bg-card rounded-3xl p-5 sm:p-7 border border-border shadow-card space-y-4">
                  <label className="flex items-start gap-3 cursor-pointer">
                    <Checkbox
                      ...
                      onCheckedChange={(c) =>
                        setCgInterviewConducted(c === true)
                      }
                      className="mt-0.5"
                    />
                    <span className="text-sm leading-relaxed">
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    <Checkbox
                      ...
                      onCheckedChange={(c) =>
                        setCgNoMisrepresentation(c === true)
                      }
                      className="mt-0.5"
                    />
                    <span className="text-sm leading-relaxed">
                      Declaro que as informacoes que forneci sobre minhas
                      qualificacoes e experiencia sao verdadeiras.
                    </span>
                  </label>
              ...
              <Button
                ...
                disabled={
                  isSubmitting ||
                  !cgInterviewConducted ||
                  !cgNoMisrepresentation ||
                  !cgPlatformAck
                }
                size="lg"
                className="w-full h-14 text-base font-semibold rounded-xl shadow-lg shadow-primary/25"
              ...
              {!interview.familyCompletedAt && (
                <div className="bg-warning/5 border border-warning/20 rounded-3xl p-5 sm:p-7">
                  <p className="text-sm text-muted-foreground flex items-center gap-2">
                    ...
                    A família ainda está avaliando a entrevista. Você será
                    notificado quando houver novidades.
                  </p>
                </div>
              )}
