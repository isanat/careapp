    ...
    <AppShell>
      <div className="h-[calc(100vh-7rem)]">
        <div className="grid h-full lg:grid-cols-[380px_1fr] gap-0">
              ...
              {isLoadingConversations ? (
                <div className="p-4 sm:p-5 md:p-7 space-y-3 sm:space-y-4">
                  {[1, 2, 3].map((i) => (
                    <Skeleton
                      key={i}
                      className="h-16 bg-secondary rounded-2xl"
                    />
                  ))}
                </div>
              ) : (
          ...
          {/* Chat Area */}
          <div
            className={`bg-card border border-border border-l-0 rounded-r-3xl rounded-l-none flex flex-col ${!mobileShowChat ? "hidden lg:flex" : "flex"}`}
          >
                  ...
                  {isLoadingMessages && isInitialLoad ? (
                    <div className="space-y-3 sm:space-y-4">
                      {[1, 2, 3].map((i) => (
                        <Skeleton
                          key={i}
                          className="h-12 w-3/4 bg-secondary rounded-2xl"
                        />
                      ))}
                    </div>
                  ) : (
                        ...
                        return (
                          ...
                          >
                            <div className="max-w-[72%]">
                              {!isOwn && (
                                <p className="text-[10px] text-muted-foreground/70 mb-1 font-display font-black">
                                  {message.senderName}
                                </p>
                              )}
                              <div
                                className={`rounded-[2rem] px-5 py-3.5 text-[1rem] leading-relaxed border ${
                                  isOwn
                              ...
                              <p
                                className={`text-[10px] text-muted-foreground/60 mt-1 ${isOwn ? "text-right" : "text-left"}`}
                              >
