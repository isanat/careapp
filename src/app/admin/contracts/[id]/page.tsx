...
function AdminContractDetailContent() {
  ...
  const handleCancel = async () => {
    ...
    try {
      const response = await apiFetch(
        `/api/admin/contracts/${params.id}/cancel`,
        {
          method: "POST",
  ...
  const handleResolveDispute = async (
    resolution: "favor_family" | "favor_caregiver" | "split",
  ) => {
    ...
    try {
      const response = await apiFetch(
        `/api/admin/contracts/${params.id}/resolve`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const timeline = [
    ...
    ...(acceptance?.familyAcceptedAt
      ? [
          {
            date: acceptance.familyAcceptedAt,
            event: "Família aceitou",
            icon: IconCheck,
          },
        ]
      : []),
    ...(acceptance?.caregiverAcceptedAt
      ? [
          {
            date: acceptance.caregiverAcceptedAt,
            event: "Cuidador aceitou",
            icon: IconCheck,
          },
        ]
      : []),
              ...
              <CardContent className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-medium">
                    {formatCurrency(contract.totalEurCents)}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="text-destructive">
                    -{formatCurrency(contract.platformFeeCents)}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center font-medium text-lg">
                  ...
                  <span className="text-success">
                    {formatCurrency(contract.caregiverAmountCents)}
                  </span>
                </div>
                    ...
                    <div className="bg-muted p-3 rounded-md">
                      ...
                      {escrow.amountReleasedCents && (
                        <div className="flex justify-between text-sm mt-1">
                          ...
                          <span>
                            {formatCurrency(escrow.amountReleasedCents)}
                          </span>
                        </div>
                      )}
                    </div>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center gap-4">
                  <Avatar className="h-14 w-14">
                    <AvatarImage
                      src={`https://api.dicebear.com/7.x/initials/svg?seed=${contract.familyName}`}
                    />
                    <AvatarFallback>
                      {contract.familyName?.charAt(0)}
                    </AvatarFallback>
                  </Avatar>
                  <div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center gap-4">
                  <Avatar className="h-14 w-14">
                    <AvatarImage
                      src={`https://api.dicebear.com/7.x/initials/svg?seed=${contract.caregiverName}`}
                    />
                    <AvatarFallback>
                      {contract.caregiverName?.charAt(0)}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="text-sm space-y-1">
                        <p>
                          ...
                          {formatDate(acceptance.familyAcceptedAt)}
                        </p>
                        <p>
                          <span className="text-muted-foreground">IP:</span>{" "}
                          <code className="text-xs bg-muted px-1 rounded">
                            {acceptance.familyIpAddress || "-"}
                          </code>
                        </p>
                        <p>
                          <span className="text-muted-foreground">
                          ...
                          <span className="text-xs">
                            {acceptance.familyUserAgent?.slice(0, 50) || "-"}
                          </span>
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="text-sm space-y-1">
                        <p>
                          ...
                          {formatDate(acceptance.caregiverAcceptedAt)}
                        </p>
                        <p>
                          <span className="text-muted-foreground">IP:</span>{" "}
                          <code className="text-xs bg-muted px-1 rounded">
                            {acceptance.caregiverIpAddress || "-"}
                          </code>
                        </p>
                        <p>
                          <span className="text-muted-foreground">
                          ...
                          <span className="text-xs">
                            {acceptance.caregiverUserAgent?.slice(0, 50) || "-"}
                          </span>
                        </p>
                      </div>
                    </div>
                ...
                {reviews.map((review) => (
                  <div key={review.id} className="p-4 border rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      <p className="font-medium">
                        {review.fromUserName} → {review.toUserName}
                      </p>
          ...
          <TabsContent value="dispute" className="space-y-6">
            <Card className="border-destructive">
              <CardHeader>
                ...
                <CardDescription>
                  Este contrato está em disputa. Resolva a situação escolhendo
              ...
              <CardContent className="space-y-6">
                <div className="bg-destructive/10 p-4 rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Uma disputa foi aberta e requer intervenção administrativa
                    para resolução.
                  </p>
                </div>
