...
function TransactionsTab() {
  ...
  const getTypeLabel = (type: string) => {
    const labels: Record<string, string> = {
      ACTIVATION: "Ativacao",
      CONTRACT_FEE: "Taxa Contrato",
      SERVICE_PAYMENT: "Pagamento Servico",
      REDEMPTION: "Resgate",
    };
  ...
  const handleRefund = async (paymentId: string) => {
    ...
    try {
      ...
      if (response.ok) fetchData();
        ...
        toast({
          title: "Erro",
          description: data.error || "Erro ao processar reembolso",
          variant: "destructive",
        });
      }
    } catch {
                  ...
                  payments.map((p) => (
                    <tr key={p.id} className="hover:bg-muted/30">
                      ...
                      <td className="px-3 py-2 text-right font-medium text-sm">
                        {formatCurrency(p.amountEurCents)}
                      </td>
  ...
  const search = async () => {
    ...
    try {
      const res = await apiFetch(
        `/api/admin/audit/customer?email=${encodeURIComponent(email.trim())}`,
      );
      if (!res.ok) {
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <AuditInfoCard
              ...
              sub={`${data.user.email} | ${data.user.role}`}
            />
            <AuditInfoCard
              icon={<IconWallet className="h-4 w-4" />}
              label="Saldo Carteira"
              value={EUR(data.wallet.balanceEurCents)}
            />
            <AuditInfoCard
              icon={<IconArrowUp className="h-4 w-4 text-success" />}
              label="Total Depositos"
              value={EUR(data.summary.totalDeposits)}
              sub={`${data.summary.completedTransactions} tx aprovadas`}
            />
            <AuditInfoCard
              icon={<IconArrowDown className="h-4 w-4 text-destructive" />}
              label="Total Saques"
              value={EUR(data.summary.totalWithdrawals)}
              sub={`Reemb: ${EUR(data.summary.refundedAmount)}`}
            />
          </div>
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold">
                    Lucro da Plataforma com este cliente
                  </span>
                </div>
                <span className="text-xl font-bold text-success">
                  {EUR(data.platformProfit.totalFeesCollected)}
                </span>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                <div className="bg-white rounded-lg p-2.5">
                  <p className="text-muted-foreground">
                    Taxas Cash-In (deposito)
                  </p>
                  <p className="font-bold text-sm">
                    {EUR(data.summary.depositFees)}
                  </p>
                </div>
                <div className="bg-white rounded-lg p-2.5">
                  <p className="text-muted-foreground">
                    Taxas Cash-Out (saque)
                  </p>
                  <p className="font-bold text-sm">
                    {EUR(data.summary.withdrawalFees)}
                  </p>
                </div>
                <div className="bg-white rounded-lg p-2.5">
                  <p className="text-muted-foreground">Taxas Contrato</p>
                  <p className="font-bold text-sm">
                    {EUR(data.summary.contractFeePlatform)}
                  </p>
                </div>
                <div className="bg-white rounded-lg p-2.5">
                  <p className="text-muted-foreground">Taxas Servico</p>
                  <p className="font-bold text-sm">
                    {EUR(data.summary.servicePaymentFees)}
                  </p>
                </div>
              </div>
              {(data.platformProfit.totalEscrowFees > 0 ||
                data.platformProfit.totalReceiptFees > 0) && (
                <div className="mt-2 text-xs text-muted-foreground">
                  Escrow: {EUR(data.platformProfit.totalEscrowFees)} | Recibos:{" "}
                  {EUR(data.platformProfit.totalReceiptFees)}
                </div>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4 pt-0">
              <div className="grid grid-cols-2 gap-x-8 gap-y-1 text-sm">
                <div className="flex justify-between py-1 border-b">
                  <span className="text-muted-foreground">
                    Entradas (depositos aprovados)
                  </span>
                  <span className="font-medium text-success">
                    +{EUR(data.crossCheck.totalIn)}
                  </span>
                </div>
                <div className="flex justify-between py-1 border-b">
                  <span className="text-muted-foreground">
                  ...
                  <span className="font-medium text-success">
                    +{EUR(data.crossCheck.tipsReceived)}
                  </span>
                </div>
                <div className="flex justify-between py-1 border-b">
                  <span className="text-muted-foreground">
                    Saidas (saques aprovados)
                  </span>
                  <span className="font-medium text-destructive">
                    -{EUR(data.crossCheck.totalOut)}
                  </span>
                </div>
                <div className="flex justify-between py-1 border-b">
                  <span className="text-muted-foreground">
                  ...
                  <span className="font-medium text-destructive">
                    -{EUR(data.crossCheck.tipsSent)}
                  </span>
                </div>
                <div className="flex justify-between py-1 border-b">
                  <span className="text-muted-foreground">
                    Taxas plataforma (total)
                  </span>
                  <span className="font-medium text-destructive">
                    -{EUR(data.crossCheck.totalFees)}
                  </span>
                </div>
                <div className="flex justify-between py-1 border-b">
                  <span className="text-muted-foreground">Reembolsos</span>
                  <span className="font-medium text-destructive">
                    -{EUR(data.crossCheck.totalRefunded)}
                  </span>
                </div>
              ...
              {!data.crossCheck.isConsistent && (
                <div className="mt-3 bg-destructive/10 rounded-lg p-3 text-sm text-destructive font-medium">
                  Diferenca: {EUR(Math.abs(data.crossCheck.difference))} (
                  {data.crossCheck.difference > 0
                    ? "cliente sacou a mais"
                    : "saldo nao contabilizado"}
                  )
                </div>
              )}
            </CardContent>
          ...
          <Tabs defaultValue="ctx">
            <TabsList className="h-9">
              <TabsTrigger value="ctx" className="text-xs">
                Transacoes ({data.payments.length})
              </TabsTrigger>
                      ...
                      <tbody className="divide-y divide-border/30">
                        {data.payments.map((p) => (
                          <tr key={p.id} className="hover:bg-muted/30">
                            ...
                            <td className="px-2 py-1.5">
                              <Badge
                                className={`${auditStatusColors[p.status] || "bg-muted"} border-0 text-[9px] px-1.5 py-0`}
                              >
                                {p.status}
                              </Badge>
                            </td>
                            <td className="px-2 py-1.5 text-right font-medium">
                              {EUR(p.amountEurCents)}
                            </td>
                            <td className="px-2 py-1.5 text-right text-warning font-medium">
                              {p.platformFee > 0 ? EUR(p.platformFee) : "-"}
                            </td>
                            <td className="px-2 py-1.5 text-right font-medium">
                              {EUR(p.amountEurCents - (p.platformFee || 0))}
                            </td>
                            <td className="px-2 py-1.5 truncate max-w-[120px]">
                              {p.description || p.contractTitle || "-"}
                            </td>
                          </tr>
                        ))}
                      ...
                      <tbody className="divide-y divide-border/30">
                        {data.paymentsGrouped.map((g, i) => (
                          <tr key={i} className="hover:bg-muted/30">
                            ...
                            <td className="px-2 py-1.5">
                              <Badge
                                className={`${auditStatusColors[g.status] || "bg-muted"} border-0 text-[9px] px-1.5 py-0`}
                              >
                                {g.status}
                              </Badge>
                            </td>
                            ...
                            <td className="px-2 py-1.5 text-right text-warning font-bold">
                              {g.totalFees > 0 ? EUR(g.totalFees) : "-"}
                            </td>
                          </tr>
                        ))}
                      </tbody>
                      ...
                      <tbody className="divide-y divide-border/30">
                        {data.contracts.map((c) => (
                          <tr key={c.id} className="hover:bg-muted/30">
                            ...
                            <td className="px-2 py-1.5">
                              <Badge
                                className={`${auditStatusColors[c.status] || "bg-muted"} border-0 text-[9px] px-1.5 py-0`}
                              >
                                {c.status}
                              </Badge>
                            </td>
                      ...
                      <tbody className="divide-y divide-border/30">
                        {data.ledgerGrouped.map((l, i) => (
                          <tr key={i} className="hover:bg-muted/30">
                            <td className="px-2 py-1.5">
                              <Badge
                                className={`${l.type === "CREDIT" ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"} border-0 text-[9px] px-1.5 py-0`}
                              >
                      ...
                      <tbody className="divide-y divide-border/30">
                        {(data.receipts as any[]).map((r: any) => (
                          <tr key={r.id} className="hover:bg-muted/30">
                            ...
                            <td className="px-2 py-1.5">
                              {format(new Date(r.periodStart), "dd/MM/yy", {
                                locale: pt,
                              })}{" "}
                              -{" "}
                              {format(new Date(r.periodEnd), "dd/MM/yy", {
                                locale: pt,
                              })}
                            </td>
                            <td className="px-2 py-1.5 text-right">
                            ...
                            <td className="px-2 py-1.5 text-right font-medium">
                              {EUR(r.totalAmountCents)}
                            </td>
                            <td className="px-2 py-1.5 text-right text-warning font-medium">
                              {EUR(r.platformFeeCents)}
                            </td>
                            <td className="px-2 py-1.5 text-right">
                              {EUR(r.caregiverAmountCents)}
                            </td>
                          </tr>
                        ))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <AuditInfoCard
              ...
              value={EUR(data.tipsSent.total)}
              sub={`${data.tipsSent.qty} gorjetas`}
            />
            <AuditInfoCard
              label="Gorjetas Recebidas"
              value={EUR(data.tipsReceived.total)}
              sub={`${data.tipsReceived.qty} gorjetas`}
            />
            <AuditInfoCard
              label="Pendente"
              value={EUR(data.summary.pendingAmount)}
              sub="Aguardando aprovacao"
            />
            <AuditInfoCard
              label="Falhou"
              value={EUR(data.summary.failedAmount)}
              sub="Transacoes com erro"
            />
          </div>
        </>
      )}
