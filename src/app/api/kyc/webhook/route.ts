...
export async function POST(request: NextRequest) {
  try {
    ...
    const isTestWebhook =
      request.headers.get("x-didit-test-webhook") === "true";
    ...
    const diditStatus =
      payload.status?.toUpperCase() || payload.decision?.status?.toUpperCase();
    ...
    if (payload.decision?.face_matches?.[0]?.similarity_percentage) {
      ...
      confidence = Math.round(
        payload.decision.liveness_checks[0].liveness_score * 100,
      );
    }
    ...
    const messages = {
      VERIFIED:
        "A sua verificação de identidade foi aprovada! Pode agora aceder a todos os recursos da plataforma.",
      REJECTED:
        "A sua verificação de identidade foi rejeitada. Entre em contacto com o suporte para mais informações.",
      PENDING:
        "A sua verificação de identidade está a ser analisada. Será notificado quando for concluída.",
    };
