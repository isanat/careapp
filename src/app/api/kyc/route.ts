...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      "KYC GET - Session:",
      session ? { id: session.user?.id } : "No session",
    );
...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          debug: "Missing DIDIT_APP_ID or DIDIT_API_KEY",
        },
        { status: 500 },
      );
    }
      ...
      return NextResponse.json(
        {
          ...
          debug: "Missing DIDIT_WORKFLOW_ID - get it from Didit dashboard",
        },
        { status: 500 },
      );
    }
    ...
    console.log(
      "KYC POST - Didit session created:",
      data.session_id,
      "token:",
      data.session_token,
    );
