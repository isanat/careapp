...
export async function GET(request: NextRequest) {
  try {
    ...
    const acceptedTerms = result.rows.reduce(
      (acc, row) => {
        const type = row.termsType as string;
        if (
          !acc[type] ||
          new Date(row.acceptedAt as string) > new Date(acc[type].acceptedAt)
        ) {
          acc[type] = {
      ...
      {} as Record<
        string,
        { version: string; acceptedAt: string; ipAddress: string }
      >,
    );
    ...
    return NextResponse.json({
      terms: termsStatus,
      allRequiredAccepted,
      missingRequired: termsStatus.filter(
        (t) => t.required && (!t.accepted || !t.isCurrent),
      ),
    });
  } catch (error) {
