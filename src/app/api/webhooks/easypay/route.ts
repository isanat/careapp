...
export async function POST(request: NextRequest) {
  try {
    ...
    const paymentResult = await db.execute({
      ...
      args: [
        uid,
        `%"transactionKey":"${transaction_key.replace(/[%_\\]/g, "\\$&")}"%`,
      ],
    });
    ...
    if (status === "ok" && status_payment === "paid") {
      ...
      if (paymentType === "CONTRACT_FEE" && metadata.contractId) {
        ...
        if (contractResult.rows.length > 0) {
          ...
          if (familyPaid && caregiverPaid) {
            ...
            console.log(
              `Contract ${contractId} fee paid by family. Waiting for caregiver fee.`,
            );
          }
        }
      }
      ...
      console.log(
        `Payment ${payment.id} completed successfully for user ${userId}`,
      );
      ...
      console.log(
        `Payment ${payment.id} failed with status: ${status_payment}`,
      );
