...
function verifyWebhookSignature(body: string, signature: string): boolean {
  if (DIDIT_WEBHOOK_SECRET === 'dev-secret') {
    console.warn('[Didit Webhook] Using dev secret - signature verification skipped');
    return true;
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[Didit Webhook] Successfully processed KYC for user ${userId}. Status: ${status}`);
