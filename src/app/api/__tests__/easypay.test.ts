...
vi.mock("@/lib/services/easypay", () => ({
  easypayService: {
    ...
    createMultibancoReference: (...args: unknown[]) =>
      mockCreateMultibancoReference(...args),
...
describe("POST /api/payments/easypay", () => {
  ...
  it("returns 503 when Easypay not configured", async () => {
    ...
    const res = await POST(
      createPostRequest({ type: "activation", method: "cc", amount: 35 }),
    );
  ...
  it("returns 400 for invalid method", async () => {
    ...
    const res = await POST(
      createPostRequest({ type: "activation", method: "bitcoin", amount: 35 }),
    );
    expect(res.status).toBe(400);
  });
...
describe("GET /api/payments/easypay", () => {
  ...
  it("returns 401 when not authenticated", async () => {
    ...
    const req = new NextRequest(
      "http://localhost:3000/api/payments/easypay?paymentId=pay-1",
    );
