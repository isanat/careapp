...
describe("POST /api/register", () => {
  ...
  it("registers a new FAMILY user successfully", async () => {
    ...
    expect(data.termsAccepted).toEqual([
      "terms_of_use",
      "privacy_policy",
      "mediation_policy",
    ]);
  ...
  it("returns 400 for invalid input", async () => {
    const res = await POST(
      createRequest({ name: "M", email: "bad", password: "1", role: "X" }),
    );
    const data = await res.json();
  ...
  it("creates user, profile, and terms records", async () => {
    await POST(createRequest(validBody));

    const calls = mockDb.execute.mock.calls;
    // 1: check existing user, 2: insert user, 3: insert profile, 4-6: insert 3 terms
    ...
    expect(calls[2][0].sql).toContain("INSERT INTO ProfileFamily");

    // Terms acceptance (3 records)
    expect(calls[3][0].sql).toContain("INSERT INTO TermsAcceptance");
  ...
  it("creates ProfileCaregiver for CAREGIVER role", async () => {
    ...
    const calls = mockDb.execute.mock.calls;
    expect(calls[2][0].sql).toContain("INSERT INTO ProfileCaregiver");
  });
