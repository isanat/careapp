...
vi.mock('@/lib/validations/schemas', () => ({
  forgotPasswordSchema: {
    safeParse: vi.fn().mockReturnValue({ success: true, data: { email: 'test@test.com' } }),
  },
  resetPasswordSchema: {
    safeParse: vi.fn().mockReturnValue({
      success: true,
      data: { token: 'test-token', email: 'test@test.com', password: 'NewPass123!' },
    }),
  },
}));
...
const mockForgotSchema = forgotPasswordSchema.safeParse as ReturnType<typeof vi.fn>;
const mockResetSchema = resetPasswordSchema.safeParse as ReturnType<typeof vi.fn>;
...
describe('POST /api/auth/forgot-password', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockForgotSchema.mockReturnValue({ success: true, data: { email: 'test@test.com' } });
  });
  ...
  it('returns 400 when email invalid (safeParse fails)', async () => {
    ...
    const res = await forgotPasswordPOST(createPostRequest({ email: 'invalid' }));
  ...
  it('returns success even when email does not exist (anti-enumeration)', async () => {
    ...
    const res = await forgotPasswordPOST(createPostRequest({ email: 'nonexistent@test.com' }));
    const data = await res.json();
  ...
  it('generates token and calls sendPasswordResetEmail', async () => {
    // User found
    mockDb.execute
      .mockResolvedValueOnce({ rows: [{ id: 'user-1', email: 'test@test.com' }] }) // SELECT user
      .mockResolvedValueOnce({ rows: [] }) // DELETE existing tokens
      .mockResolvedValueOnce({ rows: [] }); // INSERT new token

    const res = await forgotPasswordPOST(createPostRequest({ email: 'test@test.com' }));
    const data = await res.json();
...
describe('POST /api/auth/reset-password', () => {
  ...
  it('returns 400 when input invalid', async () => {
    ...
    const res = await resetPasswordPOST(createPostRequest({ token: '', email: '', password: '' }));
    const data = await res.json();
  ...
  it('returns 400 when token not found', async () => {
    ...
    const res = await resetPasswordPOST(createPostRequest({
      token: 'invalid-token',
      email: 'test@test.com',
      password: 'NewPass123!',
    }));
  ...
  it('returns 400 when token expired', async () => {
    ...
    mockDb.execute
      .mockResolvedValueOnce({
        rows: [{ identifier: 'test@test.com', token: 'hashed-token', expires: expiredDate }],
      })
  ...
  it('updates password and deletes token on success', async () => {
    ...
    mockDb.execute.mockResolvedValueOnce({
      rows: [{ identifier: 'test@test.com', token: 'hashed-token', expires: futureDate }],
    });
