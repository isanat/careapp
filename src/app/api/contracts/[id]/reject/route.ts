...
export async function POST(
  ...
  try {
    ...
    if (contract.caregiverUserId !== session.user.id) {
      return NextResponse.json(
        { error: "Apenas o cuidador pode recusar esta proposta" },
        { status: 403 },
      );
    }
    ...
    if (
      !["PENDING_ACCEPTANCE", "DRAFT", "PENDING"].includes(
        String(contract.status),
      )
    ) {
      return NextResponse.json(
        { error: "Este contrato não pode mais ser recusado" },
        { status: 400 },
      );
    }
