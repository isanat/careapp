  ...
  try {
    ...
    for (const testUser of TEST_USERS) {
      ...
      const existingUserResult = await db.execute({
        sql: `SELECT u.id, u.email, u.name, u.role, w.address, w.balanceTokens
              FROM User u
      ...
      if (existingUserResult.rows.length > 0) {
        ...
        results.created.push({
          ...
          initialTokens: Number(existingUser.balanceTokens) || 0,
          isNew: false,
        });
      ...
      if (testUser.role === "FAMILY") {
        ...
        await db.execute({
          sql: `INSERT INTO ProfileCaregiver 
                (id, userId, country, title, experienceYears, services, hourlyRateEur, bio, verificationStatus, totalContracts, totalHoursWorked, averageRating, totalReviews, createdAt, updatedAt)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
          args: [
            profileId,
            userId,
      ...
      await db.execute({
        sql: `INSERT INTO TokenLedger 
              (id, userId, type, reason, amountTokens, amountEurCents, description, createdAt)
