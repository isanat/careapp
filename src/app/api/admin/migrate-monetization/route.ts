...
export async function POST(request: NextRequest) {
  ...
  try {
    await db.execute({
      sql: `CREATE INDEX IF NOT EXISTS idx_receipt_contract ON Receipt(contractId)`,
      args: [],
    });
    await db.execute({
      sql: `CREATE INDEX IF NOT EXISTS idx_receipt_family ON Receipt(familyUserId, createdAt)`,
      args: [],
    });
    await db.execute({
      sql: `CREATE INDEX IF NOT EXISTS idx_receipt_caregiver ON Receipt(caregiverUserId, createdAt)`,
      args: [],
    });
  ...
  try {
    await db.execute({
      sql: `CREATE INDEX IF NOT EXISTS idx_rp_contract ON RecurringPayment(contractId)`,
      args: [],
    });
    await db.execute({
      sql: `CREATE INDEX IF NOT EXISTS idx_rp_status ON RecurringPayment(status, nextPaymentAt)`,
      args: [],
    });
    results.push({ step: "RecurringPayment indexes", success: true });
  } catch (e: any) {
    results.push({
      step: "RecurringPayment indexes",
      success: false,
      error: e.message,
    });
  }
  ...
  try {
    ...
    results.push({
      step: "Interview.caregiverQuestionnaireJson",
      success: true,
    });
  } catch (e: any) {
    ...
    results.push({
      step: "Interview.caregiverQuestionnaireJson",
      success: true,
      error: "Already exists or: " + e.message,
    });
  }
  ...
  try {
    ...
    results.push({
      step: "Interview.caregiverCompletedAt",
      success: true,
      error: "Already exists or: " + e.message,
    });
  }
