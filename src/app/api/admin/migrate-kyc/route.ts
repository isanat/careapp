...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      results.push({
        step: "check_user_table",
        success: false,
        error: String(e),
      });
      return NextResponse.json(
        { results, error: "User table check failed" },
        { status: 500 },
      );
    }
    ...
    try {
      ...
      existingColumns = columnsResult.rows.map(
        (row: any) => row.name as string,
      );
      results.push({ step: "get_existing_columns", success: true });
    } catch (e) {
      results.push({
        step: "get_existing_columns",
        success: false,
        error: String(e),
      });
      return NextResponse.json(
        { results, error: "Failed to get columns" },
        { status: 500 },
      );
    }
    ...
    const kycColumns = [
      {
        name: "kycSessionId",
        sql: "ALTER TABLE User ADD COLUMN kycSessionId TEXT",
      },
      {
        name: "kycSessionToken",
        sql: "ALTER TABLE User ADD COLUMN kycSessionToken TEXT",
      },
      {
        name: "kycSessionCreatedAt",
        sql: "ALTER TABLE User ADD COLUMN kycSessionCreatedAt TEXT",
      },
      {
        name: "kycCompletedAt",
        sql: "ALTER TABLE User ADD COLUMN kycCompletedAt TEXT",
      },
      {
        name: "kycConfidence",
        sql: "ALTER TABLE User ADD COLUMN kycConfidence INTEGER DEFAULT 0",
      },
    ];
    ...
    try {
      ...
      if (caregiverTableCheck.rows.length > 0) {
        ...
        const existingCaregiverCols = caregiverCols.rows.map(
          (row: any) => row.name as string,
        );
        ...
        const caregiverKycCols = [
          {
            name: "kycSessionId",
            sql: "ALTER TABLE ProfileCaregiver ADD COLUMN kycSessionId TEXT",
          },
          {
            name: "kycSessionCreatedAt",
            sql: "ALTER TABLE ProfileCaregiver ADD COLUMN kycSessionCreatedAt TEXT",
          },
          {
            name: "kycCompletedAt",
            sql: "ALTER TABLE ProfileCaregiver ADD COLUMN kycCompletedAt TEXT",
          },
          {
            name: "kycConfidence",
            sql: "ALTER TABLE ProfileCaregiver ADD COLUMN kycConfidence INTEGER DEFAULT 0",
          },
        ];
        ...
        for (const col of caregiverKycCols) {
          if (!existingCaregiverCols.includes(col.name)) {
            try {
              await db.execute({ sql: col.sql, args: [] });
              results.push({
                step: `add_caregiver_column_${col.name}`,
                success: true,
              });
            } catch (e: any) {
              if (e.message?.includes("duplicate column")) {
    ...
    try {
      ...
      const finalColNames = finalColumns.rows.map(
        (row: any) => row.name as string,
      );
...
export async function GET(request: NextRequest) {
  try {
    ...
    const existingKycCols = columns.filter((c: any) =>
      kycColumns.includes(c.name),
    );
    ...
    return NextResponse.json({
      ...
      hasAllKycColumns: kycColumns.every((name) =>
        columns.some((c: any) => c.name === name),
      ),
