...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (e.message?.includes("already exists")) {
        ...
        return NextResponse.json(
          { results, error: "Failed to create AdminUser table" },
          { status: 500 },
        );
      }
    }
    ...
    try {
      ...
      if (e.message?.includes("already exists")) {
        results.push({
          step: "create_admin_action_table",
          success: true,
          error: "Table already exists",
        });
      } else {
        ...
        return NextResponse.json(
          { results, error: "Failed to create AdminAction table" },
          { status: 500 },
        );
      }
    }
    ...
    const adminUserIndexes = [
      {
        ...
        sql: "CREATE INDEX IF NOT EXISTS idx_admin_user_role ON AdminUser(role)",
      },
      {
        name: "idx_admin_user_is_active",
        sql: "CREATE INDEX IF NOT EXISTS idx_admin_user_is_active ON AdminUser(isActive)",
      },
    ];
    ...
    for (const idx of adminUserIndexes) {
      try {
        ...
        results.push({
          step: `create_index_${idx.name}`,
          success: true,
          error: "Index may already exist",
        });
      }
    }
    ...
    const adminActionIndexes = [
      {
        name: "idx_admin_action_user_date",
        sql: "CREATE INDEX IF NOT EXISTS idx_admin_action_user_date ON AdminAction(adminUserId, createdAt)",
      },
      {
        name: "idx_admin_action_entity",
        sql: "CREATE INDEX IF NOT EXISTS idx_admin_action_entity ON AdminAction(entityType, entityId)",
      },
      {
        name: "idx_admin_action_created",
        sql: "CREATE INDEX IF NOT EXISTS idx_admin_action_created ON AdminAction(createdAt)",
      },
    ];
    ...
    try {
      ...
      if (
        !createdTables.includes("AdminUser") ||
        !createdTables.includes("AdminAction")
      ) {
        return NextResponse.json(
          {
            success: false,
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      migrationRequired:
        !existingTables.includes("AdminUser") ||
        !existingTables.includes("AdminAction"),
      existingTables,
      adminUserColumns,
      adminActionColumns,
      adminUserCount,
