  ...
  try {
    ...
    if (userRole === "CAREGIVER") {
      try {
        ...
        kycStatus =
          (kycResult.rows[0]?.verificationStatus as string) || "UNVERIFIED";
      } catch (e) {
    ...
    // Get wallet totals
    const walletTotals: Record<string, unknown> = {
      totalReceived: 0,
      totalSent: 0,
    };
  ...
  try {
    ...
    if (activeContracts > 0) {
      return NextResponse.json(
        {
          error:
            "Cannot delete user with active contracts. Please complete or cancel contracts first.",
        },
        { status: 400 },
      );
    }
