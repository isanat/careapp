...
const variants: Record<AlertVariant, string> = {
  default: 'bg-surface border-border/10 text-text',
  info: 'bg-accent/20 border-secondary/30 text-text',
...
export function Alert({ className, icon, variant = 'default', ...props }: AlertProps) {
  return (
    <div
      className={cn('flex items-start gap-3 rounded-[14px] border p-4 text-sm', variants[variant], className)}
      role="alert"
      {...props}
    >
