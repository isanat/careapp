...
async function checkLayoutIssues(page: any): Promise<any> {
...
async function checkAccessibility(page: any): Promise<any> {
  return await page.evaluate(() => {
    const issues = {
      missingAlt: 0,
      missingLabels: 0,
...
async function checkResponsiveness(page: any): Promise<any> {
  return await page.evaluate(() => {
    return {
...
async function captureScreenshots(page: any, pageName: string): Promise<{ desktop: string; mobile: string } | null> {
...
async function testPage(browser: any, pageTitle: string, pageUrl: string): Promise<void> {
  console.log(`\n${'='.repeat(70)}`);
  ...
  try {
    ...
    const response = await page.goto(pageUrl, {
      ...
      if (err.message.includes('ERR_CERT_AUTHORITY_INVALID')) {
        console.log('   ⚠️  Erro de certificado SSL (esperado em ambiente local)');
        return null;
      }
      throw err;
    });
    ...
    console.log(`Status HTTP: ${status || 'Carregado via erro de certificado'}`);
    ...
    if (requiresAuth) {
      console.log('⚠️  Página requer autenticação');
    ...
    if (layoutIssues.duplicateAppShells > 1) {
      ...
      console.log('   ⚠️  Nenhum AppShell encontrado');
    ...
    if (layoutIssues.overflowIssues > 0) {
      console.log(`   ⚠️  Issues de overflow: ${layoutIssues.overflowIssues}px`);
    ...
    console.log(`   Headers: ${elementCheck.headersVisible ? '✓' : '✗'}`);
    console.log(`   Botões: ${elementCheck.buttonsCount} encontrados`);
    console.log(`   Formulários: ${elementCheck.formsPresent ? '✓' : '✗'}`);
    console.log(`   Navegação: ${elementCheck.navigationVisible ? '✓' : '✗'}`);
